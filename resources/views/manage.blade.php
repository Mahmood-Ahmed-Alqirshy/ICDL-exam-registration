...
<body>
    ...
    <section class="px-5 lg:px-32 my-8">
        @if (request()->get('status'))
            <div
                class="mb-8 h-24 rounded-md bg-{{ $status[request()->get('status')][1] }}-500 text-white text-center flex justify-center items-center">
                {{ $status[request()->get('status')][0] }}</div>
        @endif
            ...
            @forelse ($exams as $exam)
                <div class=" bg-slate-200 rounded-md w-full">
                    ...
                    <div class=" flex items-center">
                        @if ($exam->strictTries()->count() < $exam->students)
                            @if ($exam->active)
                                <a href="{{ route('close', ['examSession' => $exam->id]) }}"
                                    class="text-white shadow-lg hover:shadow-md bg-sky-400 hover:bg-sky-600 transition text-xl px-5 py-3 rounded-md m-5 block w-fit">اغلاق</a>
                            @else
                                <a href="{{ route('open', ['examSession' => $exam->id]) }}"
                                    class="text-white shadow-lg hover:shadow-md bg-sky-400 hover:bg-sky-600 transition text-xl px-5 py-3 rounded-md m-5 block w-fit">فتح</a>
                            @endif
                        @endif
                        <a href="{{ route('export', ['examSession' => $exam->id]) }}"
                            class="text-white shadow-lg hover:shadow-md bg-sky-400 hover:bg-sky-600 transition text-xl px-5 py-3 rounded-md m-5 block w-fit">تصدير</a>

                    </div>
                </div>
            @empty
                <p
                    class="md:col-span-2 lg:col-span-3 text-3xl text-center h-32 text-slate-600 flex justify-center items-center">
                    لا توجد إمتحانات</p>
            @endforelse
            {{-- @for ($i = 0; $i < 5; $i++)
            <div class=" bg-slate-200 rounded-md w-full">
